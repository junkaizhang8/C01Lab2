...
app.get("/getAllNotes", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      res.status(200).json({ response: userNotes });
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!title && !content)
    {
      return res.status(400).json({ error: "Title or content or both are required."});
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      const filter = { username: decoded.username, _id: new ObjectId(noteId) };
      const updateDoc = { $set: {} };
